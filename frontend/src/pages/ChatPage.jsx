...
export default function ChatPage() {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${CHATBOT_URL}/api/chatbot/intent-classification`, {
        text: userInput,
      });
  ...
  const handleSubButton = async (label) => {
    ...
    if (["지원사업 신청 자격", "지원사업 신청", "지원금 지급"].includes(label))
  ...
  const handleRecommendProgram = async () => {
    try {
      ...
      // 로그인 된 상태이므로 백엔드(Spring Boot) api 요청
      const res = await axios.post(
        `${CHATBOT_URL}/api/chatbot/recommend-programs`,
        { username: username },
        { withCredentials: true }
      );

      // api 응답에서 program_list 추출
      const programs = res?.data?.data?.program_list ?? [];
      console.log("📦 추천 API 응답:", res.data);
      console.log("📋 program_list:", programs);
  ...
  const handleCategorySubmit = async () => {
    ...
    try {
      ...
      const res = await axios.post(
        `${CHATBOT_URL}/api/chatbot/program-info`,
        {
  ...
  const handlePlatformGuide = async (label) => {
    ...
    console.log("📌 전달된 label 값:", JSON.stringify(label));

    // ✅ 특정 label에 대해선 직접 정의한 버튼 세트 사용
    const predefinedGuides = {
      "지원사업 신청 자격": [
        "지원사업 신청하려면 회원가입 필요한가요?",
        "비회원도 지원사업 신청 가능한가요?",
      ],
      "지원사업 신청": [
        "사업자등록증이 꼭 필요한가요?",
        "신청 후 진행상태는 어떻게 확인하나요?",
        "지원사업 신청 내역 확인 방법",
      ],
      "지원금 지급": [
        "지원금은 언제 받을 수 있나요?",
    ...
    if (predefinedGuides[label]) {
      const featureButtons = predefinedGuides[label];
      setMessages((prev) => [
        ...prev,
        {
          from: "bot",
          text: `${label} 관련 질문을 선택해주세요.`,
          subButtons: featureButtons.map((q) => ({
            title: q,
            onClick: () => handlePredefinedGuideClick(q),
          })),
          time: getCurrentTime(),
        },
      ]);
    ...
    try {
      const res = await axios.post(
        `${CHATBOT_URL}/api/chatbot/platform-guide`,
  ...
  /* -----------------------------------------------------
  7-4-2. 플랫폼 기능 안내 기능의 특정 label이외의 경우 관련 질문 리스트 버튼 중 하나 클릭 시 동작
  ----------------------------------------------------- */
  // ✅ 플랫폼 기능 안내 기능의 동작 정의
  const handleFeatureClick = (featureName, guideList, setMessages) => {
    ...
    /* -----------------------------------------------------
  7-4-3. 플랫폼 기능 안내 기능의 특정 label의 경우 관련 질문 리스트 버튼 중 하나 클릭 시 동작
  ----------------------------------------------------- */
  // ✅ 플랫폼 기능 안내 기능의 동작 정의
  const handlePredefinedGuideClick = async (question) => {
    try {
      const res = await axios.post(`${CHATBOT_URL}/api/chatbot/platform-guide`, {
        question,
        n_k: 1,
      });

      const guide = res?.data?.guide_list?.[0];
      const description = guide?.description || "관련 정보를 찾지 못했습니다.";
      const stepGuide = guide?.step_guide || [];
      ...
      console.error("❌ 사전 정의된 가이드 요청 오류:", err);
      setMessages((prev) => [
        ...prev,
        { from: "bot", text: "정보를 불러오지 못했습니다.", time: getCurrentTime() },
      ]);
    }
  };
  ...
  const handleFaqSearch = async (question) => {
    try {
      const res = await axios.post(
        `${CHATBOT_URL}/api/chatbot/faq-search`,
        { question, n_k: 3 }
      );
